...
func (e ErrCorruptFiles) Error() string {
	return fmt.Sprintf("%f%% of the data file is corrupt, more than given corrupt alert threshold (%f%%). Cautiously refusing to start GEDB to prevent data loss.", math.Floor(100*e.CorruptionRate), math.Floor(100*e.CorruptAlertThreshold))
}
