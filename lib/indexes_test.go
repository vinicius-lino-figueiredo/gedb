...
func (s *IndexesTestSuite) TestInsertion() {
	...
	s.Run("ArrayFields", func() {
		...
		s.Run("RollBackAllOnConstraintViolated", func() {
			...
			s.Len(idx.GetAll(), 2)
			s.Len(idx.GetMatching("aa"), 1)
			s.Len(idx.GetMatching("bb"), 1)
			s.Len(idx.GetMatching("cc"), 0)
			s.Len(idx.GetMatching("dd"), 0)
			s.Len(idx.GetMatching("ee"), 0)
...
func (s *IndexesTestSuite) TestUpdate() {
	...
	s.Run("ArrayOfDocuments", func() {
		...
		s.Equal(idx.GetMatching("world")[0], doc1b)
		s.Len(idx.GetMatching("changed"), 1)
		s.Equal(idx.GetMatching("changed")[0], doc2b)
		s.Len(idx.GetMatching("bloup"), 1)
		s.Equal(idx.GetMatching("bloup")[0], doc3b)
	})
	...
	s.Run("RollbackArrayUpdate", func() {
		...
		s.Equal(doc1, idx.GetMatching("hello")[0])
		s.Len(idx.GetMatching("world"), 1)
		s.Equal(doc2, idx.GetMatching("world")[0])
		s.Len(idx.GetMatching("bloup"), 1)
		s.Equal(doc3, idx.GetMatching("bloup")[0])
	...
	s.Run("ReverSimpleAndBatch", func() {
		...
		s.Equal(3, idx.tree.GetNumberOfKeys())
		s.Len(idx.GetMatching("world"), 1)
		s.Equal(doc1b, idx.GetMatching("world")[0])
		s.Len(idx.GetMatching("changed"), 1)
		s.Equal(doc2b, idx.GetMatching("changed")[0])
		s.Len(idx.GetMatching("bloup"), 1)
		s.Equal(doc3b, idx.GetMatching("bloup")[0])
