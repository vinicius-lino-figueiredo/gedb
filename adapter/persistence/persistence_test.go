...
func (s *PersistenceTestSuite) TestCompactOnLoad() {
	...
	docs := []domain.Document{
		...
		data.M{"_id": "my_id_2", "a": 4, "createdAt": now, "updatedAt": now},
...
func (s *PersistenceTestSuite) TestCallAfterRemovingDatafile() {
	d1 := data.M{"_id": "my_id_1", "a": 1}
	d2 := data.M{"_id": "my_id_2", "a": 2}
	d := [][]domain.Document{
		{d1},
		{d1, d2},
	}
...
func (s *PersistenceTestSuite) TestWorkAsExpected() {
	...
	doc1 := data.M{"_id": "my_id_1", "a": "hello"}
	doc2 := data.M{"_id": "My_id_2", "a": "world"}
...
func (s *PersistenceTestSuite) TestDropDatabase() {
	...
	s.Run("ReloadAfterwards", func() {
		s.FileExists(s.testDb)
		ctx := context.Background()
		s.NoError(p.PersistNewState(ctx, data.M{"_id": "my_id", "hello": "world"}))
		s.FileExists(s.testDb)
	...
	s.Run("ResetBuffer", func() {
		...
		s.NoError(p.DropDatabase(ctx))
		docs := []domain.Document{
			data.M{"_id": "my_id_1", "hello": "world"},
			data.M{"_id": "my_id_2", "hello": "world"},
			data.M{"_id": "my_id_3", "hello": "world"},
		}
		s.NoError(p.PersistCachedDatabase(ctx, docs, nil))
		s.NoError(p.DropDatabase(ctx))
		s.NoError(p.PersistNewState(ctx, data.M{"_id": "my_id", "hi": "world"}))
		docs, _, err := p.LoadDatabase(ctx)
		s.NoError(err)
		s.Len(docs, 1)
	})
