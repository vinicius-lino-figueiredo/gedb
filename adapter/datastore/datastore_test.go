...
func (s *DatastoreTestSuite) TestRemove() {
	...
	s.Run("PersistMultipleRemoves", func() {
		...
		n, err := s.d.Remove(ctx,
			M{"a": M{"$in": []any{1, 3}}},
			domain.WithRemoveMulti(true),
		)
		s.NoError(err)
		s.Equal(int64(2), n)
...
func (s *DatastoreTestSuite) TestDropDatabase() {
	...
	s.Run("ReloadAfterwards", func() {
		s.FileExists(s.testDb)
		ctx := context.Background()
		cur, err := s.d.Insert(ctx, M{"_id": "my_id_1", "hello": "world"})
		s.NoError(err)
		s.NotNil(cur)
		s.FileExists(s.testDb)
		b, err := os.ReadFile(s.testDb)
		...
		s.NoError(s.d.DropDatabase(ctx))
		s.NoFileExists(s.testDb)
		cur, err = s.d.Insert(ctx, M{"_id": "my_id_2", "hello": "world"})
		s.NoError(err)
		s.NotNil(cur)
		s.NoError(err)
		s.NotNil(cur)
	...
	s.Run("ResetBuffer", func() {
		...
		s.NoError(s.d.DropDatabase(ctx))
		docs := []any{
			M{"_id": "my_id_1", "hello": "world"},
			M{"_id": "my_id_2", "hello": "world"},
			M{"_id": "my_id_3", "hello": "world"},
		}
		cur, err := s.d.Insert(ctx, docs...)
		s.NoError(err)
		s.NotNil(cur)
		s.NoError(s.d.DropDatabase(ctx))
		cur, err = s.d.Insert(ctx, M{"_id": "my_id_4", "hi": "world"})
		s.NoError(err)
		s.NotNil(cur)
