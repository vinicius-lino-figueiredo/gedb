...
func (s *IndexesTestSuite) TestInsertion() {
	...
	s.Run("PointersCompound", func() {
		...
		helloWorld, err := idx.Tree.Search(data.M{"tf": "hello", "tg": "world"})
		s.NoError(err)
	...
	s.Run("ArrayFields", func() {
		...
		s.Run("OneEntryPerElement", func() {
			...
			all := slices.Collect(idx.GetAll())
			s.Len(all, 2)
			s.Equal(obj, all[0])
			s.Equal(obj, all[1])

			s.NoError(idx.Insert(ctx, obj2))
			s.Equal(3, i.GetNumberOfKeys())
		})
		...
		s.Run("RemoveIndexAtAllUniqueElements", func() {
			...
			s.NoError(idx.Insert(ctx, obj))
			s.NoError(idx.Insert(ctx, obj2))
			aaMatches, err := listMatching(idx.GetMatching("aa"))
			s.NoError(err)
			s.Len(aaMatches, 2)
			...
			ccMatches, err := listMatching(idx.GetMatching("cc"))
			s.NoError(err)
			s.Len(ccMatches, 1)
		...
		s.Run("RollBackAllOnConstraintViolated", func() {
			...
			s.NoError(err)
			s.Len(aaMatches, 1)
			bbMatches, err := listMatching(idx.GetMatching("bb"))
			s.NoError(err)
			s.Len(bbMatches, 1)
			ccMatches, err := listMatching(idx.GetMatching("cc"))
			s.NoError(err)
			s.Len(ccMatches, 0)
			ddMatches, err := listMatching(idx.GetMatching("dd"))
			s.NoError(err)
			s.Len(ddMatches, 0)
			eeMatches, err := listMatching(idx.GetMatching("ee"))
			s.NoError(err)
			s.Len(eeMatches, 0)

			err = idx.Insert(ctx, obj2)
			s.ErrorIs(err, domain.ErrConstraintViolated)
			s.Equal(2, i.GetNumberOfKeys())
			aaMatches, err = listMatching(idx.GetMatching("aa"))
			s.NoError(err)
			s.Len(aaMatches, 1)
			bbMatches, err = listMatching(idx.GetMatching("bb"))
...
func (s *IndexesTestSuite) TestUpdate() {
	...
	s.Run("ArrayOfDocuments", func() {
		...
		s.Equal(3, idx.GetNumberOfKeys())
		worldMatches, err := listMatching(idx.GetMatching("world"))
		s.NoError(err)
		s.Len(worldMatches, 1)
		s.Equal(worldMatches[0], doc1b)
		changedMatches, err := listMatching(idx.GetMatching("changed"))
		s.NoError(err)
		s.Len(changedMatches, 1)
		s.Equal(changedMatches[0], doc2b)
		blopMatches, err := listMatching(idx.GetMatching("bloup"))
		s.NoError(err)
		s.Len(blopMatches, 1)
	...
	s.Run("RollbackArrayUpdate", func() {
		...
		s.Equal(3, idx.GetNumberOfKeys())
		helloMatches, err := listMatching(idx.GetMatching("hello"))
		s.NoError(err)
		...
		s.Equal(3, idx.GetNumberOfKeys())
		helloMatches2, err := listMatching(idx.GetMatching("hello"))
		s.NoError(err)
		s.Len(helloMatches2, 1)
		s.Equal(doc1, helloMatches2[0])
		worldMatches2, err := listMatching(idx.GetMatching("world"))
		s.NoError(err)
		s.Len(worldMatches2, 1)
		s.Equal(doc2, worldMatches2[0])
		blopMatches2, err := listMatching(idx.GetMatching("bloup"))
		s.NoError(err)
...
func (s *IndexesTestSuite) TestGetMatchingDocuments() {
	...
	s.Run("AllOrEmptyArray", func() {
		...
		nopeMatches, err := listMatching(idx.GetMatching("nope"))
		s.NoError(err)
	...
	s.Run("GetAllForNilField", func() {
		...
		yesMatches, err := listMatching(idx.GetMatching("yes"))
		s.NoError(err)
		s.Equal([]domain.Document{}, yesMatches)
		nilMatches, err := listMatching(idx.GetMatching(nil))
		s.NoError(err)
	...
	s.Run("AllWithKeyInArrayOfKey", func() {
		...
		emptyMatches, err := listMatching(idx.GetMatching())
		s.NoError(err)
		s.Equal([]domain.Document{}, emptyMatches)
		blopMatches, err := listMatching(idx.GetMatching("bloup"))
		s.NoError(err)
		s.Equal([]domain.Document{doc2}, blopMatches)
		res, err := listMatching(idx.GetMatching("bloup", "yes"))
		s.NoError(err)
		s.Equal([]domain.Document{doc2, doc4, doc5}, res)
		noMatchesNoNo, err := listMatching(idx.GetMatching("nope", "no"))
		s.NoError(err)
	...
	s.Run("AllDocsWithKeyInCertainBounds", func() {
		...
		d, err := listMatching(idx.GetBetweenBounds(ctx, data.M{"$lt": int64(10), "$gte": int64(5)}))
		s.NoError(err)
		s.Equal([]domain.Document{doc1, doc4, doc3}, d)
		d, err = listMatching(idx.GetBetweenBounds(ctx, data.M{"$lte": int64(8)}))
		s.NoError(err)
		s.Equal([]domain.Document{doc2, doc1, doc4, doc3}, d)
		d, err = listMatching(idx.GetBetweenBounds(ctx, data.M{"$gt": int64(7)}))
		s.NoError(err)
...
func (s *IndexesTestSuite) TestGetMatchingInvalidParameter() {
	...
	data, err := idx.GetMatching([]string{})
...
func (s *IndexesTestSuite) TestResetting() {
	...
	s.Run("ResetAndInitialize", func() {
		...
		s.NoError(err)
		s.Len(blopMatches, 0)
		newMatches, err := listMatching(idx.GetMatching("new"))
		s.NoError(err)
	...
	s.Run("ResetWithMultipleDocs", func() {
		...
		s.NoError(err)
		s.Equal(555, newMatches[0].Get("a"))
		againMatches, err := listMatching(idx.GetMatching("again"))
		s.NoError(err)
...
func (s *IndexesTestSuite) TestGetAll() {
	...
	s.Equal(expected, slices.Collect(idx.GetAll()))
}
...
func (s *IndexesTestSuite) TestNoField() {
	...
	docs, err := listMatching(idx.GetMatching(nil))
	s.NoError(err)
